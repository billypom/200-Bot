...
class FriendCodeCog(commands.Cog):
    ...
    async def friend_code(
        self, ctx, fc: discord.Option(str, "XXXX-XXXX-XXXX", required=False)
    ):
        if fc is None:
            await ctx.defer()
            ...
            try:
                with DBA.DBAccess() as db:
                    temp = db.query(
                        "SELECT fc FROM player WHERE player_id = %s;", (ctx.author.id,)
                    )
                    await ctx.respond(temp[0][0])
            except Exception:
                await ctx.respond(
                    "``Error 12:`` No friend code found. Use ``/fc XXXX-XXXX-XXXX`` to set."
                )
        else:
            ...
            confirmation_msg = await update_friend_code(self.client, ctx.author.id, fc)
