...
class RestrictCog(commands.Cog):
    ...
    async def zrestrict(self, ctx,
        ...
        if is_chat_restricted:
            ...
            try:
                with DBA.DBAccess() as db:
                    db.execute('INSERT INTO player_punishment (player_id, punishment_id, reason, admin_id, unban_date, ban_length) VALUES (%s, %s, %s, %s, %s, %s);', (player_id, 1, reason, ctx.author.id, unban_date, ban_length))
            except Exception as e:
