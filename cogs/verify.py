...
from config import LOUNGE, SUPPORT_CHANNEL_ID, SECONDS_SINCE_LAST_LOGIN_DELTA_LIMIT
import logging
    ...
    async def verify(
        ...
        if x:
            ...
            if response:
                ...
                await ctx.respond(f'``Error 29:`` Could not re-enter the lounge. Try again later or make a <#{SUPPORT_CHANNEL_ID}> ticket for assistance.')        
            return
        else:
            pass
        ...
        if is_banned:
            ...
            await send_to_verification_log(self.client, ctx, message, verify_description)
            return
        elif is_banned == -1:
            # Wrong link probably?
        ...
        if mkc_forum_data[0] != -1:
            ...
            await send_to_verification_log(self.client, ctx, f'Error 6: {message}', verify_description)
            return
        if user_matches_list:
            verify_color = discord.Color.teal()
            await send_to_ip_match_log(self.client, ctx, message, verify_color, user_matches_list)
        # All clear. Roll out.
        ...
        if x:
            await ctx.respond(f'``Error 10:`` Oops! Something went wrong. Try again later or make a <#{SUPPORT_CHANNEL_ID}> ticket for assistance.')
