...
class SuggestCog(commands.Cog):
    ...
    @commands.slash_command(
        name="suggest",
        description="Suggest an improvement for the Lounge (use only Letters & Numbers - 1000 characters max)",
        guild_ids=LOUNGE,
    )
    async def suggest(
        self,
        ...
        if x:
            await ctx.respond(
                "Oops! There was an error with your suggestion. Try using less symbols - just use letters and numbers."
            )
            return
        try:
            with DBA.DBAccess() as db:
                db.execute(
                    "INSERT INTO suggestion (content, author_id) VALUES (%s, %s);",
                    (message, ctx.author.id),
                )
                suggestion_id = db.query(
                    "SELECT id FROM suggestion WHERE author_id = %s AND content = %s",
                    (ctx.author.id, message),
                )[0][0]  # type: ignore
            request_message = await send_to_suggestion_voting_channel(
                self.client, ctx, suggestion_id, message
            )
            request_message_id = request_message.id
            with DBA.DBAccess() as db:
                db.execute(
                    "UPDATE suggestion SET message_id = %s WHERE id = %s;",
                    (request_message_id, suggestion_id),
                )
            ...
            await ctx.respond(
                f"`Error 81:` There was an issue with your suggestion. Please create a ticket in {SUPPORT_CHANNEL_ID} with your error number."
            )
            return
