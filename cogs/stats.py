    ...
    async def stats(self, ctx,
            ...
            if season is None:
                ...
                try:
                    if season in [5, 6]:
                        ...
                        await send_raw_to_debug_channel(self.client, f'/stats invalid season {season}: ', 'lol')
                        await ctx.respond('Invalid season')
                        return
                    stats_db = f's{int(season)}200lounge'
                except Exception as e:
                    await send_raw_to_debug_channel(self.client, 'Stats parse season db exception: ', e)
                    await ctx.respond('Invalid season')
                    return
        # Tier picker
        if tier is None:
            ...
            try:
                ...
                await send_raw_to_debug_channel(self.client, f'/stats invalid tier: {tier}', e)
                await ctx.respond("Invalid tier")
                return
        ...
        if player is None:
            ...
            if bad:
                ...
                await send_to_verification_log(self.client, ctx, player, vlog_msg.error1)
                return
            # Retrieve player ID
            try:
                with DBA.DBAccess(stats_db) as db:
        ...
        if tier is None:
            with DBA.DBAccess(stats_db) as db:
                sql = 'SELECT pm.mmr_change, pm.score, pm.mogi_id FROM player_mogi pm JOIN mogi m ON pm.mogi_id = m.mogi_id WHERE pm.player_id = %s AND m.mogi_format IN (%s) ORDER BY m.create_date DESC LIMIT %s;' % (
                ...
                for i in range(len(temp)):
                    ...
                    if i <= 9:  # if we are at the last 10 indexes (first 10, newest first)
                        ...
                        if mmr_history[i] > 0:
                            ...
                            last_10_losses += 1
            partner_average = await get_partner_avg(self.client, my_player_id, number_of_mogis, mogi_format_string, '%', stats_db)
        elif tier_id in await get_tier_id_list():
            try:
                with DBA.DBAccess(stats_db) as db:
                    sql = 'SELECT pm.mmr_change, pm.score, pm.mogi_id FROM player_mogi pm JOIN mogi m ON pm.mogi_id = m.mogi_id WHERE pm.player_id = %s AND m.tier_id = %s AND m.mogi_format IN (%s) ORDER BY m.create_date DESC LIMIT %s;' % (
                ...
                await send_to_debug_channel(self.client, ctx, f'/stats not played in tier | {e}')
                await ctx.respond(f'You have not played in {tier_channel}')
                return
            partner_average = await get_partner_avg(self.client, my_player_id, number_of_mogis, mogi_format_string, tier_id, stats_db)
        else:
        ...
        subprocess.run(['convert', rank_filename, '-fill', rgb_flag, '-tint', '100', stats_rank_filename])
