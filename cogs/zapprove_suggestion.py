...
class ApproveSuggestionCog(commands.Cog):
    ...
    async def approve(
        ...
        try:
            ...
            await send_raw_to_debug_channel(self.client, '/approve_suggestion error 1', e)
        try:
            with DBA.DBAccess() as db:
                db.execute('UPDATE suggestion SET reason = %s, was_accepted = %s, admin_id = %s WHERE id = %s', (reason, 1, ctx.author.id, suggestion_id))
        except Exception as e:
            await send_raw_to_debug_channel(self.client, '/approve_suggestion error 2', e)
        ...
        response = await handle_suggestion_decision(self.client, suggestion_id, suggestion, author_id, message_id, admin_id, 1, reason)
        if response:
