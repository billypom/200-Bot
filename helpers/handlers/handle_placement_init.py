...
async def handle_placement_init(
    ...
    with DBA.DBAccess() as db:
        ...
        db.execute(
            ...
            (my_player_mmr, init_rank, player[0]),
        )
    # Assign rank role
    try:
        ...
        await results_channel.send(
            f"<@{player[0]}> has been placed at {placement_name} ({my_player_mmr} MMR)"
        )
    except Exception as e:
        await send_raw_to_debug_channel(
            client, f"<@{player[0]}> did not stick around long enough to be placed", e
        )
    ...
    try:
        ...
        ) = await handle_queued_mmr_penalties(player[0], my_player_mmr)
        # disclosure
        if total_queued_mmr_penalty == 0:
            pass
        else:
            await results_channel.send(
                f"<@{player[0]}> accumulated {total_queued_mmr_penalty} worth of MMR penalties during placement.\nMMR adjustment: ({my_player_mmr} -> {my_player_new_queued_strike_adjusted_mmr})"
            )
    except Exception as e:
        await send_raw_to_debug_channel(
            client,
