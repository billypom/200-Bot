...
async def create_queue_channels_and_categories(client, number_of_players, groups_of_12):
    ...
    LOUNGE_QUEUE_CATEGORY_POSITION = config["LOUNGE"].getint(
        "LOUNGE_QUEUE_CATEGORY_POSITION"
    )
    # each group = channel
    ...
    for group in groups_of_12:
        if channel_count % 25 == 0:  # 25 channels per category
            ...
            try:
                with DBA.DBAccess() as db:
                    db.execute(
                        "INSERT INTO lounge_queue_category (category_id) VALUES (%s)",
                        (category_id,),
                    )
            except Exception as e:
                logging.warning(
                    f"Exception inserting specific category to lounge_queue_category table: {e}"
                )
                return False
        ...
        if lounge_staff is None:
            logging.warning(
                "create_queue_channels_and_categories oops | oops no staff in channel omg"
            )
        lounge_staff_permissions = PermissionOverwrite(
        ...
        try:
            send_room_string = (
                f"**{channel_name} MMR: {average_mmr} - tier-{room_tier_name}**\n"
                + player_room_initialization_string
            )
        except Exception as e:
        ...
        try:
            with DBA.DBAccess() as db:
                db.execute(
                    "INSERT INTO lounge_queue_channel (channel_id, category_id, average_mmr, max_mmr, min_mmr) VALUES (%s, %s, %s, %s, %s);",
                    (channel_id, category_id, average_mmr, max_mmr, min_mmr),
                )
        except Exception as e:
            logging.warning(
                f"Exception inserting channel into lounge_queue_category | {e}"
            )
        ...
        for player in player_list:
            ...
            try:
                ...
                logging.warning(
                    f"create_queue_channels_and_categories error | could not remove player from lounge_queue_player | {e}"
                )
        ...
        format_vote_view = FormatVote(
            client,
            player_list,
            clean_pingable_player_list,
            channel_id,
            average_mmr,
            min_mmr,
            max_mmr,
        ...
        vote_tasks_info.append(
            (format_vote_task, format_vote_view, channel_id, host_user_id, host_fc)
        )
    ...
    for _, format_vote_view, channel_id, host_user_id, host_fc in vote_tasks_info:
        ...
        room_response_string, list_response_string = await create_teams(
            client,
            format_vote_view.uid_list,
            vote_result,
            average_mmr,
            format_vote_view.min_mmr,
            format_vote_view.max_mmr,
            format_vote_view.channel_name,
            host_user_id,
            host_fc,
        )
        ...
        await channel.send(
            f"Open room at: <t:{room_open_time}:t>\nPenalty at: <t:{penalty_time}:t>"
        )
