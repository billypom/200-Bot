...
import re
import json
from constants import JAZZY_MKC_NAME, JAZZY_MKC_PASSWORD
...
def mt_mkc_request_registry_info(mkc_player_id):
    ...
    try:
        mkcresponse = requests.get(
            "https://www.mariokartcentral.com/mkc/api/registry/players/"
            + str(mkc_player_id),
            headers=headers,
        )
        ...
        return_value = [
            mkc_data_dict["user_id"],
            mkc_data_dict["country_code"],
            mkc_data_dict["is_banned"],
        ]
        return return_value
    except Exception:
        return [-1, -1, -1]
