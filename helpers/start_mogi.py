...
async def start_mogi(client, ctx):
    ...
    if removal_passed:
        ...
        await send_to_debug_channel(
            client, ctx, "Failed to remove all players from other lineups..."
        )
        return 0
    ...
    try:
        with DBA.DBAccess() as db:
            db.execute(
                "UPDATE tier SET voting = 1 WHERE tier_id = %s;", (ctx.channel.id,)
            )
    except Exception as e:
        await send_to_debug_channel(
            client, ctx, f"start_mogi cannot start format vote | 1 | {e}"
        )
        await channel.send(
            f"`Error 23:` Could not start the format vote. Contact the admins or {PING_DEVELOPER} immediately"
        )
        return 0
    ...
    try:
        with DBA.DBAccess() as db:
            db.execute(
                "UPDATE lineups SET mogi_start_time = %s WHERE tier_id = %s ORDER BY create_date ASC LIMIT 12;",
                (datetime.datetime.now(), ctx.channel.id),
            )
    except Exception as e:
        await send_to_debug_channel(
            client, ctx, f"start_mogi cannot start format vote | 4 | {e}"
        )
        await channel.send(
            f"`Error 23.2:` Could not start the format vote. Contact the admins or {PING_DEVELOPER} immediately"
        )
        return 0
    ...
    try:
        with DBA.DBAccess() as db:
            temp = db.query(
                "SELECT player_id FROM lineups WHERE tier_id = %s ORDER BY create_date ASC LIMIT %s;",
                (ctx.channel.id, MAX_PLAYERS_IN_MOGI),
            )
    except Exception as e:
        await send_to_debug_channel(
            client, ctx, f"start_mogi cannot start format vote | 2 | {e}"
        )
        await channel.send(
            f"`Error 22:` Could not start the format vote. Contact the admins or {PING_DEVELOPER} immediately"
        )
        return 0
    ...
    try:
        with DBA.DBAccess() as db:
            db.execute(
                "UPDATE lineups SET can_drop = 0 WHERE tier_id = %s ORDER BY create_date ASC LIMIT %s;",
                (ctx.channel.id, MAX_PLAYERS_IN_MOGI),
            )
    except Exception as e:
        await send_to_debug_channel(
            client, ctx, f"start_mogi cannot start format vote | 3 | {e}"
        )
        await channel.send(
            f"`Error 55:` Could not start the format vote. Contact the admins or {PING_DEVELOPER} immediately"
        )
        return 0
    response = ""
    ...
    with DBA.DBAccess() as db:
        unix_temp = db.query(
            "SELECT UNIX_TIMESTAMP(create_date) FROM lineups WHERE tier_id = %s ORDER BY create_date ASC LIMIT %s;",
            (ctx.channel.id, MAX_PLAYERS_IN_MOGI),
        )
    # returns the index of the voted on format, and a dictionary of format:voters
    poll_results = await check_for_poll_results(
        ctx.channel.id, unix_temp[MAX_PLAYERS_IN_MOGI - 1][0]
    )
    if poll_results[0] == -1:
        # Cancel Mogi
