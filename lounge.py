...
async def ztable(
    ...
    if bad:
        await send_to_verification_log(ctx, scores, discord.Color.blurple(), vlog_msg.error1)
        await ctx.respond(f'``Error 56:`` Invalid input. There must be 12 players and 12 scores.')
        return
    ...
    if len(score_list) == 24:
        ...
        await ctx.respond(f'``Error 57:`` Invalid input. There must be 12 players and 12 scores.')
        return
    ...
    has_dupes = await check_for_dupes_in_list(player_list_check)
    if has_dupes:
        await ctx.respond('``Error 58:`` You cannot have duplicate players on a table')
        return
    ...
    if mogi_format == 1:
        ...
        await ctx.respond(f'``Error 59:`` Invalid format: {mogi_format}. Please use 1, 2, 3, 4, or 6.')
        return
    ...
    for team in chunked_list:
        ...
        for player in team:
            try:
                ...
                await ctx.respond(f'``Error 60:`` There was an error with the following player: <@{player[0]}>. If this is a sub, you must first use the `/sub` command')
                return
        # print(team_score)
        if count == 0:
            count = 1
    ...
    if mogi_score == 984:
        ...
        await ctx.respond(f'``Error 61:`` `Scores = {mogi_score} `Scores must add up to 984.')
        return
            ...
            for idx, player in enumerate(team):
                ...
                try:
                    with DBA.DBAccess() as db:
                        ...
                        db_mogi_id = temp[0][0]
                        # Insert reference record
                        db.execute('INSERT INTO player_mogi (player_id, mogi_id, place, score, prev_mmr, mmr_change, new_mmr, is_sub) VALUES (%s, %s, %s, %s, %s, %s, %s, %s);', (player[0], db_mogi_id, int(my_player_place), int(my_player_score), int(my_player_mmr), int(my_player_mmr_change), int(my_player_new_mmr), 0))
                        # Update player record
                        db.execute('UPDATE player SET mmr = %s WHERE player_id = %s;', (my_player_new_mmr, player[0]))
                        # Remove player from lineups
                        # db.execute('DELETE FROM lineups WHERE player_id = %s AND tier_id = %s;', (player[0], ctx.channel.id)) # YOU MUST SUBMIT TABLE IN THE TIER THE MATCH WAS PLAYED
...
async def get_partner_avg(uid, number_of_mogis, tier_id=str('%')):
    try:
        with DBA.DBAccess() as db:
            ...
            # temp = db.query('SELECT AVG(score) FROM (SELECT player_id, mogi_id, place, score FROM player_mogi WHERE player_id <> %s AND (mogi_id, place) IN (SELECT pm.mogi_id, pm.place FROM player_mogi as pm JOIN mogi as m on pm.mogi_id = m.mogi_id WHERE pm.player_id = %s AND tier_id like %s ORDER BY m.create_date DESC LIMIT %s)) as table2;', (uid, uid, tier_id, number_of_mogis))
            temp = db.query("SELECT AVG(score) FROM (SELECT pm.player_id, pm.mogi_id, pm.place, pm.score FROM player_mogi as pm INNER JOIN (SELECT pm.mogi_id, pm.place FROM player_mogi as pm JOIN mogi as m on pm.mogi_id = m.mogi_id WHERE pm.player_id = %s AND tier_id like '%s' ORDER BY m.create_date DESC LIMIT %s) as pm2 ON pm2.mogi_id = pm.mogi_id WHERE player_id <> %s) as a", (uid, uid, tier_id, number_of_mogis))
            try:
