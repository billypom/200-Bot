...
async def table(
    ...
    if table_view.value is None:
        ...
        correct = subprocess.run(['convert', '-background', 'None', '-fill', 'white', pango_string, 'mkbg.png', '+swap', '-compose', 'Multiply', '-composite', '-quality', '100', mmr_filename], check=True, text=True)
        # '-fill', '#00000050', '-draw', 'rectangle 0,0 570,368'
